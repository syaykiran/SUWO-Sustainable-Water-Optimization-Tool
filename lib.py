...
def gemplot_scatter_collect(sim_outputs, obs_data, node_names, month_names, lower_limit=None, upper_limit=None, initial=None):
    ...
    # Colors and Labels
    model_colors = ["#993399", "#377EB8", "#FF7F00"]  # Normal (Purple), Wet (Blue), Dry (Orange)
    model_dark_colors = ["#993399", "#377EB8", "#FF7F00"]  # Same for dark markers
    model_labels = ["Normal Year", "Wet Year", "Dry Year"]

    for ax_idx, (ax, node_name) in enumerate(zip(axs, node_names)):
        node_idx = np.where(node_names == node_name)[0][0]

        # **Ensure Normal Year is Plotted Last for Visibility**
        for model_idx in [1, 2, 0]:  # Wet, Dry, then Normal Year
            ...
            boxprops = dict(linestyle='-', linewidth=1.2, alpha=0.4, facecolor="none", edgecolor=model_colors[model_idx])
            capprops = dict(linewidth=1.2, alpha=0.4, color=model_colors[model_idx])
            whiskerprops = dict(linewidth=1.2, alpha=0.4, color=model_colors[model_idx])
            medianprops = dict(linewidth=1.2, linestyle='-', alpha=0.4, color=model_colors[model_idx])
            meanprops = dict(marker='o', markersize=9, markerfacecolor=model_colors[model_idx], alpha=0.2)
            ...
            # **Scatter Plot**
            ax.scatter(np.arange(0, 13), sim, color=model_dark_colors[model_idx], marker='o', alpha=0.8, s=10, zorder=5)
            ...
            # 🔥 **Prevent Negative & Excessive Values**
            smoothed_data = np.clip(smoothed_data, 0, upper_limit[node_idx])

            # **Plot Smoothed Curve**
            ax.plot(smoothed_months, smoothed_data, color=model_colors[model_idx], linestyle='-', alpha=0.7, linewidth=1.5, zorder=4)

            # Set the y-axis label and legend only for the first subplot
            ax.set(ylabel=None)

            if ax_idx == 0:
                ax.set_ylabel("Reservoir Storage (hm$^3$)")


        # **Plot Constraints at the End (Behind Everything)**
        if lower_limit is not None and upper_limit is not None and initial is not None:
            ...
            ax.axhspan(lower_limit_val, lower_limit[node_idx][0], facecolor='lightgrey', alpha=0.3, zorder=0)
            ax.axhspan(upper_limit[node_idx][0], upper_limit_val, facecolor='lightgrey', alpha=0.3, zorder=0)
    ...
    fig.legend(handles=handles, title="Hydrological Conditions", loc="lower center", bbox_to_anchor=(0.5, -0.1),
               ncol=len(model_labels), frameon=False)
...
def plot_2d_pareto_compare(RES_F1, RES_F2, color_1_hues, color_2_hues):
    ...
    for i in range(3):
        ...
        # Define classification axis for each plot
        if i == 0:  # Plot 1: Classify based on Pot. Energy Def. (index 1)
            classes_1 = classify_data(RES_F1, 1)
            classes_2 = classify_data(RES_F2, 1)
        elif i == 1:  # Plot 2: Classify based on Irrigation Def. (index 2)
            classes_1 = classify_data(RES_F1, 2)
            classes_2 = classify_data(RES_F2, 2)
        elif i == 2:  # Plot 3: Classify based on Dev. from Natural Flow (index 0)
            classes_1 = classify_data(RES_F1, 0)
            classes_2 = classify_data(RES_F2, 0)

        # Plot each class for RES_F1
        for j, class_indices in enumerate(classes_1):
            ax.scatter(
                RES_F1[class_indices, (i + 1) % 3],
                RES_F1[class_indices, (i + 2) % 3],
                s=9,
                alpha=0.8,
                marker="o",
                color=color_1_hues[j],
                label=f"NSGA-II Class {j+1}" if i == 0 and j == 0 else "",
                edgecolors="none",
            )

        # Plot each class for RES_F2
        for j, class_indices in enumerate(classes_2):
            ax.scatter(
                RES_F2[class_indices, (i + 1) % 3],
                RES_F2[class_indices, (i + 2) % 3],
                s=9,
                alpha=0.8,
                marker="o",
                color=color_2_hues[j],
                label=f"NSGA-III Class {j+1}" if i == 0 and j == 0 else "",
                edgecolors="none",
            )

        ax.set_xlabel(
            [
                "Dev. from Natural Flow",
                "Pot. Energy Def.",
                "Irrigation Def.",
            ][(i + 1) % 3]
        )
...
def parallel_coordinates_plot(data1, data2, color_1_hues, color_2_hues):
    ...
    ynames = ['Pot. Energy Def.', 'Irrigation Def.', 'Dev. from Natural Flow']
