import numpy as np
from lib import gemplot_scatter_single, get_data_string, plot_boxplot_outflow, plot_scatter_collect, plot_boxplot_collect, \
...
total_iter_1, iter_name_1, coef_values_1 = read_penalty_coef("EXP_NSGA3_PEN_COEF")
...
for idx in range(total_iter_1):
    ...
    dfs = [pd.read_csv(f"EXP_RES_X_NSG3_CLMTAVOF3_1.csv").iloc[:, 1:] for i in range(1, 4)]
...
SIM_STORAGES = [SIM_STORAGES_1]
SIM_IRG_WATERS = [SIM_IRG_WATER_1]
...
gemplot_scatter_single(SIM_STORAGES, OBS_STO[E], NAME_LIST[E], MONTHS_NO, S_MIN[E], S_MAX[E], S_0[E])
