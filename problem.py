...
def corrected_rank_from_fronts(fronts, n):
    ...
    rank = np.full(n, sys.maxsize, dtype=int)  # Changed 1e16 to 1000000
...
class SakaryaOptimization(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        RESULT = simulation_model(DEC_VARS_0, self.K_1, self.K_2, self.K_3,
                                 self.K_SC)
...
def main():
    pop_size = 300
    n_gen = 300
    scenario_code = "NSG3_CLMTAVOF3"  # Define the scenario name "NSG3_SEED6"
    ...
    for idx, (K_1, K_2, K_3, K_SC) in enumerate(coef_values, start=1):
        ...
        for gen_num, algo in enumerate(res.history):
            pd.DataFrame(algo.opt.get("X")).to_csv(
                f"NEW_HIST_FILES/hist_X_{scenario_code}_{idx}_gen_{gen_num}.csv", index=False
            )
            pd.DataFrame(algo.opt.get("F")).to_csv(
                f"NEW_HIST_FILES/hist_F_{scenario_code}_{idx}_gen_{gen_num}.csv", index=False
            )
    ...
    # Export data to CSV
    df_X.to_csv(f"NEW_HIST_FILES/ALL_RES_X_{scenario_code}.csv", index=False)
    df_F.to_csv(f"NEW_HIST_FILES/ALL_RES_F_{scenario_code}.csv", index=False)
